        ...
        char** argv)
{
    ...
    const char* topic_xml_1_2 = "<dds>"
            "<topic>"
            "<name>HelloWorldTopic_2_to_3</name>"
            "<dataType>HelloWorld</dataType>"
            "</topic>"
            "</dds>";
    ...
    uint16_t requests[] = {
        ...
        datareader_req, topic_req_mas, subscriber_req_mas, datareader_req_mas
    };
   ...
   srand((unsigned) time(&t));
    ...
    while (connected)
    {
       ...
       sprintf(topic.message, "Publisher C(1) random value = %d", rand() % 8);
